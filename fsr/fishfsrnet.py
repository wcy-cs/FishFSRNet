...
class FISHNET(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        self.down1 = nn.Sequential(*common.invUpsampler_module(2, n_feats, act=False))

        self.down_stage1 = nn.Sequential(
            *fish_block(args))
        self.down2 = nn.Sequential(*common.invUpsampler_module(2, n_feats, act=False))

        self.down_stage2 = nn.Sequential(
            *fish_block(args))
        self.down3 = nn.Sequential(*common.invUpsampler_module(2, n_feats, act=False))
        ...
        self.up21 = nn.Sequential(*common.Upsampler(2, n_feats))

        self.conv_tail2 = nn.Conv2d(in_channels=n_feats * 2, out_channels=n_feats, kernel_size=1, stride=1)
        self.up2_stage1 = nn.Sequential(
            *fish_block(args))
        self.up22 = nn.Sequential(*common.Upsampler(2, n_feats))  # n_feats*3

        self.up2_stage2 = nn.Sequential(
            *fish_block(args))
        self.up23 = nn.Sequential(*common.Upsampler(2, n_feats))
